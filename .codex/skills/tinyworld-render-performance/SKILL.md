...
- `fadeMatCache` shares fade materials in `FADE_BUCKETS = 16` opacity buckets keyed by (base material UUID, grayscale flag, bucket, keepFadeAtOpaque). `prepareFadeable` and `applyElementOpacity` look up via `pickFadeMaterial(baseMat, grayscale, displayOpacity, keepFadeAtOpaque)` instead of cloning per mesh. Flat terrain tile roots set `keepFadeAtOpaque` so top panels remain on the transparent/depthWrite-off fade material even at 100%; snapping those tops back to the base opaque material exposes diagonal face artifacts that are absent at 99% opacity. Raised terrain is the exception: its complete shell returns to base materials at full visibility so vertical walls look solid and opaque caps depth-occlude additive shields behind the elevation. Individual riser meshes also set `userData.opaqueAtFullOpacity`. Cached materials are tagged `userData.cachedFade = true` and must never be mutated or disposed — they're shared by every mesh in their bucket. If you need a per-instance opacity (e.g. squash anim), clone the material yourself and tag it so it gets disposed individually.
...
- Flat `BASE_TERRAIN` on the playable home board uses persistent `homeTileBuckets`: dirt/riser and cap `InstancedMesh` pairs split across a small deterministic stone-shade palette so the board is not uniformly grey. `renderCellTile` updates only the affected cell slot and keeps a lightweight per-cell detail root for metadata. Raised and non-default/path/water/terraform terrain stays on the detailed per-cell path so adjacency and edge visuals remain intact. Initialize/clear the buckets on full load/reset, resolve desktop/XR picks through each mesh's `keyAt[instanceId]`, and never rebuild these buckets in the animation loop.
...
- Stats overlay (`?stats=1` or backtick key) reads `renderer.info` and reports FPS, draws, tris, geoms, mats, programs, textures, ghost-board count + queue depth. While open, it also estimates visible object draw contribution by cell kind/building type by counting descendant meshes; this is diagnostic attribution rather than an exact per-object GPU counter because renderer frustum culling is applied later. Use it to measure any rendering change.
...
- Starfields belong in the 3D scene as fixed world-space `THREE.Points` on a large hollow spherical shell around the playable center, not a camera-frustum seed, flat wall, cylinder, or below-ground layer. Keep the inner radius far outside the world/camera movement area (currently roughly 520+ units, with 200+ units of center clearance) and keep the main camera far plane large enough to include the far side of the shell. Do not use CSS/DOM star layers or camera/target-following star groups. Keep depth testing enabled and render stars early/non-transparent with `depthWrite: false` so transparent/fade terrain, buildings, ghost boards, and neighbour islands draw over stars instead of letting stars visually pass through foreground geometry.
...
- Lighting stack: `AmbientLight` (flat fill so shadowed sides never go
  ...
  now that there is no post pass. Time of day changes the sun/hemisphere
  colours and sky but intentionally keeps the neutral intensity baseline so
  night remains readable and moonlit; weather may still attenuate that
  baseline.
