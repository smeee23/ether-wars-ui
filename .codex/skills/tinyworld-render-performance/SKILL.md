...
- Flat `BASE_TERRAIN` on the playable home board uses persistent `homeTileBuckets`: one dirt/riser `InstancedMesh` and one cap `InstancedMesh`. `renderCellTile` updates only the affected cell slot and keeps a lightweight per-cell detail root for metadata. Raised and non-default/path/water/terraform terrain stays on the detailed per-cell path so adjacency and edge visuals remain intact. Initialize/clear the buckets on full load/reset, resolve desktop/XR picks through each mesh's `keyAt[instanceId]`, and never rebuild these buckets in the animation loop.
